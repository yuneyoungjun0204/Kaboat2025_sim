...
class VRXONNXControllerV7(Node):
    """VRX ONNX 모델 기반 제어 노드 v7 - 회전 방향 및 X축 기준 차이 고려한 좌표계 변환"""
    
    def __init__(self):
        super().__init__('vrx_onnx_controller_v7')
        ...
        self.get_logger().info('🚢 VRX ONNX Controller v7 시작! (회전 방향 및 X축 기준 차이 고려한 좌표계 변환)')
    ...
    def check_obstacles_in_forward_path(self):
        ...
        obstacle_threshold = 30.0  # 40m 임계값
        has_obstacles = False
        
        for i in range(start_index, end_index):
            obstacle_threshold=obstacle_threshold/math.sqrt(math.sqrt(abs(i)))
    ...
    def calculate_direct_heading(self, current_pos, target_pos):
        """v5 추가: atan을 이용한 직접 헤딩 계산 (Unity NED 좌표계, 회전 방향 고려)"""
