...
class UnifiedPlotManager:
    ...
    def update_obstacle_check_area(self, area_points: List[List[float]]):
        """장애물 검사 영역 시각화 (Convex Hull Polygon)

        Args:
            area_points: [[East, North], ...] 리스트 (GPS 관례)
        """
    ...
    def update_goal_check_areas(self, goal_areas: List[dict]):
        ...
        """
        self.logger.info(f'🟣 Updating goal areas: {len(goal_areas)} areas')

        for area in goal_areas:
            if 'corners' in area and len(area['corners']) >= 4:
                corners = np.array(area['corners'])
                self.logger.info(f'🟣 Drawing goal polygon with corners: {corners}')
    ...
    def update_los_target(self, los_target: List[float], robot_pos: List[float]):
        ...
        # 연결선
        line, = self.ax_main.plot(
            [robot_pos[0], los_target[1]],  # X=North
            [robot_pos[1], los_target[0]],  # Y=East
...
class UnifiedVizNode(Node):
    ...
    def los_callback(self, msg):
        """LOS target 콜백"""
        if len(msg.data) >= 2:
            self.los_target = [msg.data[0], msg.data[1]]  # [East, North] (GPS 관례)
        else:
    ...
    def obstacle_callback(self, msg):
        ...
        self.obstacle_check_area = [
            [msg.data[i], msg.data[i + 1]]  # [East, North] (GPS 관례)
            for i in range(0, len(msg.data) - 1, 2)
        ]

    def goal_callback(self, msg):
        """Goal 체크 영역 콜백"""
        self.get_logger().info(f'📦 Goal callback: len={len(msg.data)}, data={list(msg.data)[:10]}')

        if len(msg.data) < 9 or msg.data[0] != 4.0:
            self.goal_check_areas = []
            return

        # msg.data 형식: [4.0, x1, y1, x2, y2, x3, y3, x4, y4]
        # 원본 데이터가 (x=North, y=East) 순서로 온다고 가정
        corners = [
            [msg.data[i], msg.data[i+1]]  # [North, East]
            for i in range(1, len(msg.data), 2)
        ]

        self.get_logger().info(f'🎯 Goal corners: {corners}')

        if len(corners) >= 4:
            self.goal_check_areas = [{'corners': corners}]
            self.get_logger().info(f'✓ Goal check areas set: {len(corners)} corners')
        else:
            self.goal_check_areas = []
            self.get_logger().warn(f'⚠ Not enough corners: {len(corners)}')
