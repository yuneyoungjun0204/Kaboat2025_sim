...
class DetectionSystem:
    ...
    def __init__(self, depth_estimator, device="cuda", detection_threshold=None,
                 ...
                 spatial_smoothing=True, spatial_kernel_size=5, frame_skip=1):
        """
        Args:
            ...
            frame_skip: 프레임 스킵 (N 프레임마다 1번 탐지, 기본 1=모든 프레임, 2=2프레임마다 1번)
        """
        self.device = device
    ...
    def __init__(self, depth_estimator, device="cuda", detection_threshold=None,
        ...
        Args:
            ...
            spatial_smoothing: 공간적 depth smoothing 활성화 여부
            spatial_kernel_size: spatial smoothing 커널 크기 (홀수 권장)
            frame_skip: 프레임 스킵 (N 프레임마다 1번 탐지, 기본 1=모든 프레임)
        """
        self.device = device
        self.depth_estimator = depth_estimator
        self.frame_skip = frame_skip
    ...
    def detect_objects(self, image: np.ndarray, mission_type: MissionType) -> List[Dict]:
        ...
        # 프레임 스킵 (성능 최적화: N 프레임마다 1번만 탐지)
        self.frame_counter += 1
        if self.frame_skip > 1 and self.frame_counter % self.frame_skip != 0:
            return self.last_detections  # 마지막 결과 재사용
        ...
        # === 2. Detection 전처리 (해상도 축소로 성능 최적화) ===
