...
class VRXSystemFactory:
    ...
    def create_detection_system(
        ...
        if enable_preprocessing:
            # Detection용: 해상도 축소 (성능 최적화: 50-200ms → 20-60ms)
            # 'max_speed': 416x320 (최대 속도), 'balanced': 640x480 (균형)
            image_preprocessor = create_preprocessor('max_speed')  # 416x320으로 축소
            ...
            self.logger.info("✓ 전처리 활성화: Detection=416x320(최적화), Depth=416x320(4배 축소)")
