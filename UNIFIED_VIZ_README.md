...
`trajectory_viz.py`는 모든 좌표계를 **NED(North-East-Down)**로 완전히 통일하여 더욱 직관적이고 명확한 시각화를 제공합니다. PX4와 동일한 좌표계를 사용합니다.

## 주요 개선사항

### 1. 좌표계 완전 통일
- **모든 데이터를 NED 좌표로 통일**: GPS, LiDAR, 웨이포인트, 장애물 영역 등
- **CoordinateTransformer 클래스**: 모든 좌표 변환을 한 곳에서 관리
- **명확한 좌표 규칙**: X=North, Y=East (PX4 표준 NED 좌표계)
...
이는 PX4 Autopilot의 표준 좌표계와 동일합니다:
- **Body Frame**: FRD (Forward, Right, Down)
- **World Frame**: NED (North, East, Down)

### 내부 데이터 형식
```python
position = [North, East]  # 모든 위치 데이터
waypoint = [North, East]  # 웨이포인트
los_target = [North, East]  # LOS target
obstacle_area = [[North1, East1], [North2, East2], ...]  # 영역 점들
```

### ROS2 메시지 변환
```python
# GPS 데이터 (입력)
gps_data = {'utm_x': North, 'utm_y': East}  # GPS 관례 (UTM)
position = [gps_data['utm_x'], gps_data['utm_y']]  # NED 순서

# 웨이포인트 발행 (출력)
msg = Point(x=North, y=East, z=0.0)  # NED 좌표계
```
...
| 특징 | 설명 |
|------|------|
| 좌표계 통일성 | 완전 통일 (NED - PX4 표준) |
| 장애물 영역 표시 | Polygon + 외곽선 + 중심점 |
| 코드 구조 | 중앙집중식 (CoordinateTransformer) |
| 화면 크기 | 18x9 inch |
| LiDAR 표시 | 3곳 (메인 + 극좌표 + 제어출력) |
| 성능 | 최적화 (draw_idle 사용) |
| 에러 처리 | 강화 (Fallback 포함) |
...
@staticmethod
def custom_to_ned(data) -> np.ndarray:
    # 변환 로직
